...
def citation_verification_prompt(
    item_type: str,
    citation: str,
    current_metadata: dict,
    content: str,
    ...
    """
    Generate-then-compare prompt for verifying bibliographic metadata.

    Two-part structure prevents anchoring bias: the LLM first generates
    the correct reference from content alone (Step 1), then compares its
    findings against Zotero's data (Step 2). Both steps are in a single
    prompt/call.

    Args:
        item_type: Current Zotero item type (e.g., "journalArticle", "webpage")
        citation: Rendered APA citation from Zotero API (plain text)
        current_metadata: Dict of current field values (field_name -> value)
        content: Source content (first ~15K chars)
    ...
    Returns:
        Formatted prompt string
    """
    # Format current metadata for display (no annotations)
    metadata_lines = []
    for field, value in current_metadata.items():
        metadata_lines.append(f"  {field}: {value}")
    metadata_display = "\n".join(metadata_lines)

    return f"""You are a bibliographic metadata specialist. This is a two-step task.

═══ STEP 1: GENERATE REFERENCE FROM CONTENT ═══

Read the source content below and determine the correct bibliographic fields.

Source Content (may be truncated):
{content}

Based ONLY on the source content above, determine:

GENERATED_TYPE: <the correct Zotero item type>
GENERATED_AUTHORS: <real author(s) from byline, "by" line, or author metadata — LastName, FirstName format for persons; plain name for organizations>
GENERATED_DATE: <publication date in YYYY-MM-DD, YYYY-MM, or YYYY>
GENERATED_TITLE: <the article/document title, without site name suffixes>
GENERATED_PUBLICATION: <publication/website/journal name>

Guidelines for Step 1:
- For authors: look for bylines ("by ...", "written by ...", author metadata).
  Distinguish between the AUTHOR (person who wrote it) and the PUBLICATION (where it appeared).
  "BBC News" is a publication, not an author. "The Walrus" is a magazine, not an author.
- For dates: extract the most specific date you can find. Prefer YYYY-MM-DD.
- For titles: use the article's actual headline, not including " | SiteName" or " - SiteName" suffixes.
- For item type: use Zotero types (webpage, newspaperArticle, magazineArticle, blogPost, report, journalArticle, etc.)

═══ STEP 2: COMPARE AND PROPOSE FIXES ═══

Now compare your generated reference against Zotero's current data:

Zotero's Rendered APA Citation:
{citation}
...
Compare your Step 1 findings against the Zotero citation and metadata above.
Propose field-level corrections for any discrepancies.

CITATION_STATUS: <correct | needs_fixes>
CITATION_ISSUES: <brief description of what's wrong, or "none">

ITEM_TYPE_ASSESSMENT:
CURRENT: {item_type}
SUGGESTED: <your GENERATED_TYPE, or same as current if correct>
CONFIDENCE: <high | medium | low>
REASON: <brief explanation>

Then for each field that needs correction, provide a block like this:

FIELD: <field_name>
STATUS: <confirmed | corrected | extracted | not_found>
VALUE: <the correct value from your Step 1 analysis>
CONFIDENCE: <high | medium | low>

Rules:
- For creators: "LastName, FirstName" for persons; plain name for organizations (e.g., "BBC News"). Never split org names into person fields.
- For dates: use YYYY-MM-DD. Do NOT reduce a specific date to just a year unless the day/month is wrong.
- Never replace a real person's name with a company/website name.
- If you cannot determine a field from the content, use STATUS: not_found.
- If the citation is correct, respond with just CITATION_STATUS: correct and CITATION_ISSUES: none.
- You do NOT need to include a FIELD block for every field. Only include blocks for fields that are wrong, missing, or that you want to explicitly confirm.
