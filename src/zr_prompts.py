...
def citation_verification_prompt(
    ...
    """
    Citation-driven prompt for verifying bibliographic metadata.

    Shows the rendered APA citation alongside current metadata and source
    content. The LLM evaluates whether the citation is correct and proposes
    field-level fixes.
    ...
    return f"""You are a bibliographic metadata specialist. Your task is to evaluate whether the APA citation below is correct by comparing it against the source content, and propose field-level corrections if needed.

══════════════════════════════════════════════════════
RENDERED APA CITATION (generated by Zotero):
{citation}
══════════════════════════════════════════════════════
...
1. Compare the rendered APA citation against the source content. Look for:
   - Wrong or missing author/creator (e.g., website name used as author instead of actual byline)
   - Wrong or missing date (e.g., "n.d." when the article has a clear publication date)
   - Wrong or incomplete title
   - Missing or wrong publication/website name
   - Missing or wrong DOI/URL
   - Wrong item type (e.g., "document" when it should be "webpage" or "newspaperArticle")

2. Start your response with CITATION_STATUS and CITATION_ISSUES.

3. If the citation has problems, assess the item type and propose field-level corrections.

4. For creators/authors:
   - Provide as "LastName, FirstName" separated by semicolons for multiple authors
   - If the author is an organization, provide just the organization name (e.g., "BBC News", NOT "News, BBC")
   - If the current author is actually a website/publication name, look for the real author byline in the content
   - Do NOT simply reformat the current value — find the genuinely correct value from the content
   - If you cannot find the real author, use STATUS: not_found

5. For dates: use YYYY-MM-DD format if possible, YYYY-MM if only month available, or YYYY if only year.
...
STATUS meanings:
- confirmed: current value is correct (only include for fields you explicitly checked)
- corrected: current value was wrong, providing correct value
- extracted: field was missing/empty, extracted from content
- not_found: could not determine value from content

IMPORTANT: You do NOT need to include a FIELD block for every field. Only include blocks for fields that are wrong, missing, or that you want to explicitly confirm. If the citation is correct, you can respond with just CITATION_STATUS: correct and CITATION_ISSUES: none.
